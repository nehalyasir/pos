...
// ALTER TABLE `tec_payments` ADD `doctype` INT NOT NULL AFTER `store_id`;
...
class Sales extends MY_Controller
{
    ...
    public function get_opened_list()
    {
        ...
        if ($this->db->dbdriver == 'sqlite3') {
            $this->datatables->select("id,hold_ref refid, date, customer_name, hold_ref, (total_items || ' (' || total_quantity || ')') as items, grand_total", false);
        } else {
            $this->datatables->select("id,hold_ref refid, date, customer_name, hold_ref, CONCAT(total_items, ' (', total_quantity, ')') as items, grand_total", false);
        }
    ...
    public function get_sales()
    {
        $this->load->library('datatables');
        if ($this->db->dbdriver == 'sqlite3') {
            $this->datatables->select("id, strftime('%Y-%m-%d %H:%M', date) as date, customer_name, total, total_tax, total_discount, grand_total, paid, status, hold_ref refid");
        } else {
            $this->datatables->select("id, DATE_FORMAT(date, '%Y-%m-%d %H:%i') as date, customer_name, total, total_tax, total_discount, grand_total, paid, status, hold_ref refid");
        }
    ...
    { 
        ...
        $this->datatables->from('sale_return');
        if (!$this->Admin && !$this->session->userdata('view_right')) {
            $this->datatables->where('created_by', $this->session->userdata('user_id'));
        }
        $this->datatables->where('store_id', $this->session->userdata('store_id'));
        $this->datatables->add_column('Actions', "<div class='text-center'><div class='btn-group'><a href='" . site_url('pos/view_return/$1/196/1') . "' title='" . lang('view_invoice') . "' class='tip btn btn-primary btn-xs' data-toggle='ajax-modal'><i class='fa fa-list'></i></a> <a href='" . site_url('sales/returnpayments/$1') . "' title='" . lang('view_payments') . "' class='tip btn btn-primary btn-xs' data-toggle='ajax'><i class='fa fa-money'></i></a> <a href='" . site_url('sales/add_payment/$1') . "' title='" . lang('add_payment') . "' class='tip btn btn-primary btn-xs' data-toggle='ajax'><i class='fa fa-briefcase'></i></a> <a href='" . site_url('sales/deleteReturn/$1') . "' onClick=\"return confirm('" . lang('alert_x_sale') . "')\" title='" . lang('delete_sale') . "' class='tip btn btn-danger btn-xs'><i class='fa fa-trash-o'></i></a></div></div>", 'id');
    ...
    {
        $this->data['error']      = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['page_title'] = lang('Return_pos');
        $bc                       = [['link' => '#', 'page' => lang('Return_pos')]];
        $meta                     = ['page_title' => lang('Return_pos'), 'bc' => $bc];
        $this->page_construct('sales/List_sale_retrun', $this->data, $meta);
    }
    ...
    /* -------------------------------------------------------------------------------- */
	 ...
	 public function returnpayments($id = null)
    {
        $this->data['payments'] = $this->sales_model->getSalereturnPayments($id);
	...
	public function get_sale_order_sync() { // sarfaraz
    $this->load->model('sales_model');
    $output = $this->sales_model->get_allsale_order_sync();
    $data=json_decode($output);
    echo $output;
    }
	public function sale_order_sync() // sarfaraz
    {
        $this->data['error']      = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['page_title'] = lang('Return_pos');
        $bc                       = [['link' => '#', 'page' => lang('Sale_Order_Sync')]];
        $meta                     = ['page_title' => lang('Order_Sync_List'), 'bc' => $bc];
        $this->page_construct('sales/sale_order_sync', $this->data, $meta);
    }
